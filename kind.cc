...
WCMSP::Token::Token(WCMSP::Kind kind, string lexeme) 
 {
 	if(kind == ID){
 		if(lexeme == "return") {this->kind = WCMSP::RETURN;}
 		else if(lexeme == "if") {this->kind = WCMSP::IF;}
        else if(lexeme == "else") {this->kind = WCMSP::ELSE;}
        else if(lexeme == "while") {this->kind = WCMSP::WHILE;}
        else if (lexeme == "println") {this->kind = WCMSP::PRINTLN;}
        else if (lexeme == "wain") {this->kind= WCMSP::WAIN;}
        else if (lexeme == "int") {this->kind = WCMSP::INT;}
        else if (lexeme =="new") {this->kind = WCMSP::NEW;}
        else if (lexeme == "delete") {this->kind = WCMSP::DELETE;}
        else if (lexeme == "NULL") {this->kind = WCMSP::_NULL;}
        else {this->kind = kind;}
