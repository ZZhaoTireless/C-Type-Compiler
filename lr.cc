...
int main()
{
	...
	while (cin >> ITM)
	{
		...
		{
			if (check(*it, VEC_TH.back(), ITM, "reduce", 1))
				OPCOND = 1;
		}
		T_code = 0;
		for (int i = 0; i < 10; i++)
		...
		while (OPCOND)
		{
			...
			T = L;
			while (L--)
				if (check(VEC_TR[T - L - 1], VEC_TH.back(), ITM, "reduce", 1))
					IN_R = VEC_TR[T - L - 1].PATH;
			T_code = 0;
			for (vector<string>::iterator it = MAP_R[IN_R].begin(); it != MAP_R[IN_R].end(); ++it)
				cout << *it << " ";
			cout << endl;
			L = MAP_R[IN_R].size() - 1;
			T = L;
			...
			T = L;
			while (L--)
				if (check(VEC_TR[T - L - 1], VEC_TH.back(), MAP_R[IN_R][0], "", 0))
					NEX = VEC_TR[T - L - 1].PATH;
		...
		while (L--)
			if (check(VEC_TR[T - L - 1], VEC_TH.back(), ITM, "", 0))
				NEX = VEC_TR[T - L - 1].PATH;
		for (int i = 0; i < 10; i++)
		{
			T_code++;
...
{
	if (Mode == 1)
		return (samp.IPT == IPT_V && samp.ITS == ITS_V && samp.OPE == OPE_V);
