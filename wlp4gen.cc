...
struct paraTree
{
	...
	~paraTree()
	{
		for (vector<paraTree *>::iterator it = subTR_vec.begin(); it != subTR_vec.end(); it++)
			delete *it;
	};
	string Rl_str;
...
void crt_Pro_TBL(paraTree *TR_POT, string PRO_Key)
{
	...
	else
	{
		...
		if ((front == "ID") && dup)
		{
			...
			for (it = PRO_vec.begin(); it != PRO_vec.end(); it++)
				if (*it == item)
					dup = false;
			if (dup)
				throw error;
	...
	int len = TR_POT->subTR_vec.size();
	for (int i = 0; i < len; ++i)
		crt_Pro_TBL(TR_POT->subTR_vec[i], PRO_Key);
};
...
void crt_TBL(paraTree *TR_POT)
{
	...
	{
		for (int i = 0; i < 3; i++)
			crt_Pro_TBL(TR_POT->subTR_vec[arr[i]], PRO_Key);
	}
	if (if_w)
		crt_Pro_TBL(TR_POT->subTR_vec[0], "wain");

	int len = TR_POT->subTR_vec.size();
	for (int i = 0; i < len; ++i)
		crt_TBL(TR_POT->subTR_vec[i]);
}
...
{
	if (sel == 0)
		cout << "mult $" << fir << ", $" << sec << endl;
	if (sel == 1)
		cout << "div $" << fir << ", $" << sec << endl;
	if (sel == 2)
		cout << "mfhi $" << fir << endl;
	if (sel == 3)
		cout << "mflo $" << fir << endl;
...
string getTY_check(paraTree *TR_POT, string PRO_Key)
{
	...
	else if (first_w == "expr")
	{
		...
		else if (third_w == "MINUS")
		{
			...
			if (PRO_K2 && PRO_K3)
				TY_check = "int";
			else if (!(PRO_K2 || PRO_K3))
				TY_check = "int";
			else if (PRO_K3 || (!PRO_K2))
				TY_check = "int*";
			else
				throw err;
		}
